...
const typeMap = {
  restaurant: "Food", cafe: "Food", bar: "Food", meal_takeaway: "Food",
  food: "Food", bakery: "Food", meal_delivery: "Food",
  grocery_or_supermarket: "Grocery", supermarket: "Grocery",
  hospital: "Healthcare", pharmacy: "Healthcare", doctor: "Healthcare",
  health: "Healthcare", drugstore: "Healthcare",
  store: "Retail", shopping_mall: "Retail", clothing_store: "Retail",
  electronics_store: "Retail", department_store: "Retail",
  local_government_office: "Government", post_office: "Government",
  city_hall: "Government", courthouse: "Government",
};
...
const estimateWait = (rating, userRatingsTotal, priceLevel) => {
  let base = 10;
  if (rating >= 4.5) base += 20;
  else if (rating >= 4.0) base += 10;
  if (userRatingsTotal > 1000) base += 10;
  if (priceLevel >= 3) base += 10;
  return Math.min(base + Math.floor(Math.random() * 10), 75);
};
...
const WaitTypeModal = ({ location, onClose, onReport }) => {
  ...
  const baseWait = location.wait;
  const dineInWaits = { 1: Math.round(baseWait * 0.8), 2: baseWait, 3: Math.round(baseWait * 1.3), 4: Math.round(baseWait * 1.6), 5: Math.round(baseWait * 2), 6: Math.round(baseWait * 2.4) };
          ...
          <>
            ...
            {(type === "pickup" || type === "delivery") && (
              <div style={{ background: "#0f2a1e", border: "1px solid #00e5a030", borderRadius: 14, padding: "14px 16px", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 32, color: waitColor(baseWait).text }}>{baseWait}<span style={{ fontSize: 13, color: "#555", fontWeight: 400 }}>m</span></div>
              </div>
            )}
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, marginBottom: 20 }}>
              {[1, 2, 3, 4, 5, 6].map(size => {
                ...
                return (
                  <button key={size} onClick={() => setPartySize(size)} style={{ background: isSelected ? "#00e5a022" : "#1e2535", border: `2px solid ${isSelected ? "#00e5a0" : "#2a3244"}`, borderRadius: 14, padding: "14px 10px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
            ...
            {partySize && (
              <div style={{ background: "#0f2a1e", border: "1px solid #00e5a030", borderRadius: 14, padding: "16px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <div style={{ fontSize: 13, color: "#00e5a0", fontWeight: 600 }}>🪑 Table for {partySize}</div>
                  <div style={{ fontSize: 12, color: "#555", marginTop: 2 }}>Estimated wait</div>
                </div>
...
export default function Home() {
  ...
  const fetchNearbyPlaces = useCallback(async (lat, lng) => {
    setLoading(true);
    try {
      const types = ["restaurant", "cafe", "grocery_or_supermarket", "hospital", "store", "local_government_office", "post_office", "pharmacy"];
      const allPlaces = [];
      ...
      const mapped = unique.slice(0, 20).map((p, i) => {
        const category = getCategory(p.types);
        const wait = estimateWait(p.rating || 3.5, p.user_ratings_total || 100, p.price_level || 1);
        const trends = ["up", "down", "stable"];
        const distMeters = Math.sqrt(Math.pow((p.geometry.location.lat - lat) * 111000, 2) + Math.pow((p.geometry.location.lng - lng) * 111000, 2));
  ...
  const handleWaitType = ({ type, partySize }) => {
    ...
    if (type === "dinein" && partySize) {
      ...
      const wait = Math.round(base * multipliers[partySize]);
      showToast(`🪑 Table for ${partySize}: ~${wait} min wait`);
    } else if (type === "pickup") {
      showToast(`🥡 Pickup wait: ~${loc.wait} min`);
    } else if (type === "delivery") {
      showToast(`🛵 Driver pickup: ~${loc.wait} min`);
    }
  };
      ...
      <div style={{ padding: "20px 16px 0" }}>
        ...
        {locationError && !loading && (
          <div style={{ background: "#1e1a14", border: "1px solid #ff5c5c33", borderRadius: 16, padding: "20px", textAlign: "center", marginBottom: 20 }}>
            ...
            <button onClick={() => { setLocationError(null); setLoading(true); window.location.reload(); }} style={{ background: "#00e5a0", border: "none", borderRadius: 10, padding: "10px 20px", color: "#0d1117", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>Try Again</button>
          </div>
        )}
