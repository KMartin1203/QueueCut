...
const estimateDriverWait = (rating, totalRatings, priceLevel) => {
  ...
  let base = 5;
  if (totalRatings > 2000) base += 10;
  else if (totalRatings > 500) base += 5;
  if (isPeak) base += 8;
  if (priceLevel >= 3) base += 4;
  if (rating >= 4.5) base += 5;
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 14, color: "#fff", marginBottom: 3 }}>{place.name}</div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <button onClick={() => onReport(place)} style={{ background: "transparent", border: "1px solid #2a3244", borderRadius: 8, padding: "5px 12px", color: "#00e5a0", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>
          + Report
        </button>
      </div>
...
const ReportCard = ({ report }) => (
  ...
  >
    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 }}>
      <div style={{ flex: 1 }}>
        <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 14, color: "#fff", marginBottom: 4 }}>{report.location}</div>
    ...
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      ...
      {report.verified && <span style={{ fontSize: 11, color: "#00e5a0" }}>✓ Verified driver</span>}
    </div>
  </div>
);
...
const SubmitModal = ({ onClose, onSubmit, userLocation }) => {
  ...
  const handleSubmit = () => {
    ...
    setSubmitted(true);
    setTimeout(() => { onSubmit({ platform, location: selectedPlace.displayName, address: selectedPlace.formattedAddress, wait: waitRange.value, note }); }, 1500);
  };

  const canProceed = (step === 1 && platform) || (step === 2 && selectedPlace && waitRange !== null) || step === 3;
          ...
          <>
            ...
            {step === 1 && (
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  {platforms.map(p => (
                    <button key={p.id} onClick={() => setPlatform(p.id)} style={{ background: platform === p.id ? `${p.color}22` : "#1e2535", border: `1.5px solid ${platform === p.id ? p.color : "#2a3244"}`, borderRadius: 14, padding: "14px 12px", color: platform === p.id ? p.color : "#888", cursor: "pointer", fontSize: 13, fontWeight: platform === p.id ? 700 : 400, display: "flex", alignItems: "center", gap: 8, transition: "all 0.15s" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div>
                  ...
                  {selectedPlace ? (
                    ...
                    <div style={{ position: "relative" }}>
                      <input value={locationSearch} onChange={e => setLocationSearch(e.target.value)} placeholder="e.g. McDonald's, Chipotle..." style={{ width: "100%", background: "#1e2535", border: "1px solid #2a3244", borderRadius: 12, padding: "12px 14px", color: "#fff", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
                ...
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    {waitRanges.map(r => (
                      <button key={r.value} onClick={() => setWaitRange(r)} style={{ background: waitRange?.value === r.value ? "#00e5a022" : "#1e2535", border: `1.5px solid ${waitRange?.value === r.value ? "#00e5a0" : "#2a3244"}`, borderRadius: 12, padding: "11px 12px", color: waitRange?.value === r.value ? "#00e5a0" : "#888", cursor: "pointer", fontSize: 12, fontWeight: waitRange?.value === r.value ? 700 : 400, display: "flex", alignItems: "center", gap: 6, transition: "all 0.15s" }}>
                        {r.emoji} {r.label}
                      </button>
                    ))}
                  </div>
            ...
            {step === 3 && (
              <div>
                ...
                <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="e.g. Only 2 orders ahead, staff was quick, parking is tough..." rows={4} style={{ width: "100%", background: "#1e2535", border: "1px solid #2a3244", borderRadius: 12, padding: "12px 14px", color: "#fff", fontSize: 13, outline: "none", resize: "none", boxSizing: "border-box", fontFamily: "'DM Sans', sans-serif" }} />
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
              ...
              <button onClick={() => step < 3 ? setStep(s => s + 1) : handleSubmit()} disabled={!canProceed} style={{ flex: 2, background: canProceed ? "#00e5a0" : "#1e2535", border: "none", borderRadius: 12, padding: 14, color: canProceed ? "#0d1117" : "#555", fontWeight: 700, cursor: canProceed ? "pointer" : "default", fontSize: 14, transition: "all 0.15s" }}>
...
export default function DriverReports() {
  ...
  const mapPlace = useCallback((p, lat, lng) => {
  ...
  const fetchNearbyRestaurants = useCallback(async (lat, lng) => {
    ...
    try {
      ...
      const { places } = await Place.searchNearby({
        fields: ["id", "displayName", "location", "types", "rating", "userRatingCount", "priceLevel", "formattedAddress", "regularOpeningHours", "businessStatus"],
      ...
      if (places && places.length > 0) {
        const mapped = places.map(p => mapPlace(p, lat, lng));
  ...
  const searchRestaurants = useCallback(async (query) => {
    ...
    try {
      ...
      const { places } = await Place.searchByText({
        ...
        fields: ["id", "displayName", "location", "types", "rating", "userRatingCount", "priceLevel", "formattedAddress", "regularOpeningHours", "businessStatus"],
        maxResultCount: 20,
        ...(userLocation && { locationBias: { center: userLocation, radius: 10000 } }),
      });
      if (places && places.length > 0) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => searchRestaurants(search), 600);
    return () => clearTimeout(timer);
  ...
  const handleSubmit = ({ platform, location, address, wait, note }) => {
    const p = platforms.find(p => p.id === platform);
    setDriverReports(prev => [{ id: Date.now(), platform, location, address, wait, note, time: "just now", verified: false }, ...prev]);
    setNearbyRestaurants(prev => prev.map(r =>
      r.name === location ? { ...r, reports: [{ platform, wait, time: "just now" }, ...r.reports] } : r
    ));
    setShowModal(false);
    showToast(`✓ Report added for ${p?.label}!`);
  };

  const filteredReports = filterPlatform === "all" ? driverReports : driverReports.filter(r => r.platform === filterPlatform);
  const avgWait = nearbyRestaurants.length ? Math.round(nearbyRestaurants.reduce((s, r) => s + r.driverWait, 0) / nearbyRestaurants.length) : 0;
      ...
      <div style={{ padding: "20px 16px 0" }}>
        ...
        <div style={{ background: "linear-gradient(135deg, #0f2a1e, #0d1a2e)", border: "1px solid #00e5a030", borderRadius: 20, padding: "20px", marginBottom: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
            {[
              { label: "Avg Driver Wait", value: isLoading ? "..." : `${avgWait}m`, icon: "⏱" },
              { label: "Fast Pickups", value: isLoading ? "..." : fastPickups, icon: "⚡" },
              { label: "Nearby Spots", value: isLoading ? "..." : nearbyRestaurants.length, icon: "📍" },
            ].map(s => (
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {[{ id: "nearby", label: "📍 Nearby Restaurants" }, { id: "reports", label: "📣 Driver Reports" }].map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ flex: 1, background: activeTab === tab.id ? "#00e5a0" : "#161b26", border: activeTab === tab.id ? "none" : "1px solid #1e2535", borderRadius: 10, padding: "10px", fontSize: 12, fontWeight: activeTab === tab.id ? 700 : 400, color: activeTab === tab.id ? "#0d1117" : "#888", cursor: "pointer" }}>{tab.label}</button>
          ))}
        </div>
          ...
          <>
            ...
            <div style={{ position: "relative", marginBottom: 14 }}>
              ...
              <input
                ...
                placeholder="Search any restaurant (e.g. Chipotle, Wendy's)..."
                style={{ width: "100%", background: "#161b26", border: "1px solid #1e2535", borderRadius: 12, padding: "11px 36px 11px 34px", color: "#fff", fontSize: 14, outline: "none" }}
              />
              {search.length > 0 && (
                <button
                  onClick={() => { setSearch(""); if (userLocation) fetchNearbyRestaurants(userLocation.lat, userLocation.lng); }}
            ...
            {isLoading && (
              <div style={{ textAlign: "center", padding: "48px 0" }}>
                ...
                <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 16, color: "#fff", marginBottom: 6 }}>
                  {search ? `Searching for "${search}"...` : "Finding restaurants near you..."}
                </div>
            ...
            {locationError && !isLoading && (
              <div style={{ background: "#1e1a14", border: "1px solid #ff5c5c33", borderRadius: 16, padding: "20px", textAlign: "center", marginBottom: 20 }}>
                ...
                <div style={{ fontSize: 14, color: "#ff5c5c", fontWeight: 600, marginBottom: 6 }}>{search ? "No Results" : "Location Required"}</div>
                <div style={{ fontSize: 13, color: "#666", marginBottom: 16 }}>{locationError}</div>
                <button onClick={() => search ? searchRestaurants(search) : window.location.reload()} style={{ background: "#00e5a0", border: "none", borderRadius: 10, padding: "10px 20px", color: "#0d1117", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>Try Again</button>
              </div>
            )}
              ...
              <>
                <div style={{ background: "#0f1e14", border: "1px solid #00e5a020", borderRadius: 12, padding: "10px 14px", marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ fontSize: 12, color: "#666" }}>
                    {search ? `Showing driver wait times for "${search}"` : "Sorted by shortest driver wait first"}
                  </span>
                </div>
